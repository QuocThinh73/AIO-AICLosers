async function baseSearch({query, ocr, image, translate, includeBatchIds, excludeBatchIds, topK}) {
    ...
    if (includeBatchIds?.trim()) fd.append("include_batch_ids", includeBatchIds.trim());
    if (excludeBatchIds?.trim()) fd.append("exclude_batch_ids", excludeBatchIds.trim());

    const res = await fetch("/api/search/base_search", { method: "POST", body: fd });
